...
from utils import (
    collect_activations, get_command_line_args, extract_datetime_from_path,
    is_source_file, get_latest_source_file,
)
...
from hooks import (
    get_stepwise_hook, get_indywise_hook, get_hook_module,
    get_tforced_stepwise_hook,
)
...
def ensure_accuracy(
    ...
    with torch.no_grad():
        ...
        pstr = tokenizer.decode(pred_ids[0], skip_special_tokens=False)
        lstr = tokenizer.decode(out_ids[0], skip_special_tokens=False)
        str_equal = pstr==lstr
        acc = (pred_ids==out_ids).float().mean()
        is_accurate = acc>=0.995
        if verbose:
            if is_accurate:
                print("Accuracy Ensured")
            else:
                print("Accuracy Failure")
            print("Strs Equal:", str_equal)
            print("Acc:", acc)
        return is_accurate
...
def config_prep(config):
    ...
    for si,sfile in enumerate(config["source_files"]):
        if not is_source_file(sfile):
            sfile = get_latest_source_file(sfile)
            config["source_files"][si] = sfile
    ...
    for li,lay in enumerate(config["layers"]):
        if not lay: config["layers"] = "embeddings"
...
def main():
    ...
    defaults = {
        ...
        "use_train_mode": False, # If false, will keep model in eval mode
            # during training
        ...
        "lr": 1e-3,
        "max_length": None,                 # max token length for our (toy) examples
    ...
    for mi,source_file in enumerate(config["source_files"]):
        ...
        print("Using Layer Name:", config["layers"][mi])
        ...
        src_ids = source_data["full_ids"]
        output_ids = source_data["predicted_ids"] # Slightly different
        ...
        for k in sorted(list(source_data.keys())):
            try:
                print(k, type(source_data[k]), source_data[k].shape)
            except:
        ...
        perm = torch.randperm(len(src_ids)).long()

        ml = config.get("max_length", src_ids.shape[1])
        if ml is None or ml<=0: ml = src_ids.shape[1]
        ...
        # Ensure 100% model accuracy on labels
        assert ensure_accuracy(
            model=model,
            input_ids=source_data["full_ids"],
            output_ids=source_data["predicted_ids"],
            tokenizer=tokenizer,
            verbose=True,
            device=devices[-1],
        )
            ...
            for vi,varb in enumerate(config["swap_keys"][si]):
                ...
                for k in src_datasets:
                    ...
                    intrv_data = make_intrv_data_from_src_data(
                        src_input_ids=src_datasets[k][si]["input_ids"],
                        trg_input_ids=src_datasets[k][ti]["input_ids"],
                        null_varb=varb=="" or varb=="null_varb",
                        n_samples=config[f"n_{k}_samples"],
                        shuffle=config.get("shuffle_intrv_data", True),
                        src_output_ids=src_datasets[k][si]["output_ids"],
                        trg_output_ids=src_datasets[k][ti]["output_ids"],
                        trg_prompt=prompts[ti],
                        src_prompt=prompts[si],
                        src_actvs=src_datasets[k][si]["actvs"],
                        trg_actvs=src_datasets[k][ti]["actvs"],
                    ...
                    if not config.get("shuffle_intrv_data", True):
                        if config["source_files"][si]==config["source_files"][ti]:
                            print("\nChecking shuffled Intrv")
                            for kk in ["input_ids", "output_ids"]:
                                assert torch.all(
                                  intrv_data[k]["src_"+kk]==intrv_data[k]["trg_"+kk]
                                )
                                assert torch.all(
                                  intrv_data[k]["src_"+kk]==intrv_data[k]["cl_"+kk]
                                )
                        print()
                    ...
                    all_src_activations[k][dir_var_tup] = torch.FloatTensor(
                        intrv_data["src_actvs"])
        ...
        for k in intrv_datasets.keys():
            for dirvar_tup in intrv_datasets[k].keys():
                ...
                cl_vectors[k][dirvar_tup] = get_cl_vectors(
                    ...
                    input_ids=batch["cl_input_ids"],
                    output_ids=batch.get("cl_output_ids",None),
    ...
    if config.get("stepwise", True):
        hook_fns = [get_tforced_stepwise_hook(comms_dict) for _ in models]
    else:
