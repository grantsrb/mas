...
def extract_datetime_from_path(path_str):
    ...
    for match in re.finditer(pattern, path_str):
        last_match = match
    if not last_match:
        fake_path = "fake_d1900-01-01_t11-11-11"
        return extract_datetime_from_path(fake_path)
...
def contains_model_dir(file_path):
    """
    Determines if the argued file path contains safetensors somewhere
    in its subdirectories
    """
    if not os.path.exists(file_path) or not os.path.isdir(file_path):
        return False
    for root,sub_ds,files in os.walk(file_path):
        for d in sub_ds:
            if is_model_dir(os.path.join(root,d)): return True
    return False

def is_model_dir(file_path):
    """
    Determines if the argued file path is a source file path (was it
    generated by the collect activations script?).
    """
    if not os.path.exists(file_path) or not os.path.isdir(file_path):
        return False
...
def get_latest_model(model_dir):
    """
    Searches through the sub directories to find the latest hf loadable
    model folder
    """
    model_dirs = []
    for root, sub_ds, files in os.walk(model_dir):
        for d in sub_ds:
            if is_model_dir(os.path.join(root,d)):
                model_dirs.append(os.path.join(root,d))
    if len(model_dirs)==0: return model_dir
    model_dirs = sorted(model_dirs, key=extract_datetime_from_path)
    if "step-" in model_dirs[-1]:
        latest = model_dirs[-1].split("step-")[0]
        model_dirs = [md for md in model_dirs if latest in md]
        model_dirs = sorted( model_dirs, key=sort_by_step )
    return model_dirs[-1]
...
def is_source_file(file_path):
    ...
    if ".pt"==file_path[-3:] and "srcactvs_" in file_path:
        return True
    return False
...
def get_latest_source_file(file_path):
    ...
    if len(source_files)==0: return None
    source_files = sorted(source_files, key=extract_datetime_from_path)
    return source_files[-1]

if __name__=="__main__":
    print(extract_datetime_from_path("no_dt"))